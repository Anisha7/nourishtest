(function () {
  ...
  function menuService() {
    ...
    var service = {
      ...
      defaultRoles: ['user', 'admin', 'guest'], // added guest user roles
      getMenu: getMenu,
      menus: {},
    ...
    function init() {
      ...
      shouldRender = function (user) {
        // console.log((this.roles.indexOf('guest')== 1 && this.roles.length==1));
        if (this.roles.indexOf('*') !== -1 || (this.roles.indexOf('guest') == 0 && !user)) { // created the guest role
          return true;
        } else {
          if (!user) {
            return false;
